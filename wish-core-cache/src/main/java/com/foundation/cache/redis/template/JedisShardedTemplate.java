package com.foundation.cache.redis.template;
...
/**
 * JedisShardTemplate 是支持数据分配的jedisTemplate.
 * 计算key的hash值用不同的jedisPool进行处理，如果只有一个jedispool,不会进行key的hash计算，会直接进行数据存储
 * 因为进行分配存储，所以这个shardTemplage不支持管道Pipelined与事物操作,
 * 而且也不支持多键值操作， like: like mget, rpoplpush....
 * Created by fqh on 2016/7/13
 */
public class JedisShardedTemplate {
     ...
     */
    public <T> T execute(String key, JedisTemplate.JedisAction<T> jedisAction) throws JedisException {
     ...
     * @key the action must process only this key, or this key is a sharding key.
     */
    public void execute(String key, JedisTemplate.JedisActionNoResult jedisAction) throws JedisException {
        JedisTemplate jedisTemplate = getShard(key);
     ...
     * @key the action must process only this key, or this key is a sharding key.
     */
    public List<Object> execute(String key, JedisTemplate.PipelineAction pipelineAction) throws JedisException {
        JedisTemplate jedisTemplate = getShard(key);
     ...
     * @key the action must process only this key, or this key is a sharding key.
     */
    public void execute(String key, JedisTemplate.PipelineActionNoResult pipelineAction) throws JedisException {
        JedisTemplate jedisTemplate = getShard(key);
    ...
    /**
     * 如果 shardingKey 已经存在并且是一个字符串， APPEND 命令将 value 追加到 key 原来的值的末尾。
     * 如果 shardingKey 不存在， APPEND 就简单地将给定 key 设为 value ，就像执行 SET key value 一样。
     *
     * @param shardingKey
     * @param value
     * @return
     */
    public Long append(final String shardingKey, final String value) {
        JedisTemplate jedisTemplate = getShard(shardingKey);
        return jedisTemplate.append(shardingKey, value);
    }
     ...
     */
    public Long expire(String shardingKey, final int seconds) {
        JedisTemplate jedisTemplate = getShard(shardingKey);
        return jedisTemplate.expire(shardingKey.getBytes(), seconds);
    }

    /**
     * 判断key是否存在
     */
    public boolean exists(final String shardingKey) {
        JedisTemplate jedisTemplate = getShard(shardingKey);
        return jedisTemplate.exists(shardingKey);
    }
     ...
     */
    public Long ttl(final String shardingKey) {
        JedisTemplate jedisTemplate = getShard(shardingKey);
        return jedisTemplate.ttl(shardingKey);
    }
